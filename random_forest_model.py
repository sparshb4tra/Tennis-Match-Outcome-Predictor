...
def prepare_target_and_features(df):
    df['target'] = 1  # Winner row

    # Create flipped loser row
    df_loser = df.copy()
    df_loser['target'] = 0

    # Flip relevant features for the loser perspective
    df_loser = df_loser.rename(columns={
        'winner_rank': 'loser_rank',
        'loser_rank': 'winner_rank',
        'ace_pct_winner': 'ace_pct_loser',
        'ace_pct_loser': 'ace_pct_winner',
    ...
    # Feature selection
    features = [
        'rank_diff', 'ace_pct_winner', 'ace_pct_loser',
...
if __name__ == "__main__":
    ...
    from meval_rf import evaluate_model
    df_cleaned = clean_data(df)
    df_preprocessed = preprocess_data(df_cleaned)
    model, X_test, y_test, y_pred = train_random_forest_model(df_preprocessed)